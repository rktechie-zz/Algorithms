...
public class Knapsack {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int wt[] = {5, 4, 6, 3};
        int W = 10;
        System.out.println(knapsack(v, wt, W));
    }

    public static int knapsack(int v[], int wt[], int W) {
        int N = wt.length; // Get the total number of items. Could be wt.length or val.length. Doesn't matter
        int[][] val = new int[N + 1][W + 1]; //Create a matrix. Items are in rows and weight at in columns +1 on each side
